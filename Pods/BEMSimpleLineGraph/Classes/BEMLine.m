...
- (void)drawRect:(CGRect)rect {
    ...
    /* 
     ...
     Zul: Small modification since Swift has problem in carrying Unsafe data. Refer original BEMSimpleLineGraph for previous code.
     
     For bottomGradientBundled, refer on this guideline:
     
     1. First array is for return in CGColorSpaceRef
     2. Second array is for return in Integer or size_t
     3. Third array is for return in CGFloat in array, consists of 2 arrays
     4. Fourth array is for return in CGFloat in array, consists of 8 arrays
     
     */
    
    CGColorSpaceRef colorspace = (__bridge CGColorSpaceRef)(self.bottomGradientBundled[0]);
    size_t num_locations = 2;
    NSArray *locationsArray = self.bottomGradientBundled[2];
    CGFloat locations[2] = { (NSInteger)locationsArray[0], (NSInteger)locationsArray[1] };
    NSArray *componentsArray = self.bottomGradientBundled[3];
    CGFloat components[8] = {
        [NSString stringWithFormat:@"%d",componentsArray[0]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[1]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[2]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[3]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[4]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[5]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[6]].doubleValue,
        [NSString stringWithFormat:@"%d",componentsArray[7]].doubleValue
    };
    
    CGGradientRef gradBot = CGGradientCreateWithColorComponents(colorspace, components, locations, num_locations);

    if (gradBot != nil) {
        CGContextSaveGState(ctx);
        CGContextAddPath(ctx, [fillBottom CGPath]);
        CGContextClip(ctx);
        CGContextDrawLinearGradient(ctx, gradBot, CGPointZero, CGPointMake(0, CGRectGetMaxY(fillBottom.bounds)), 0);
        CGContextRestoreGState(ctx);
    }
    ...
    if(self.customLineDataToPass.count != 0) {
        
        NSLog(@"[BEMSimpleLineGraph] customLineDataToPass: %@",self.customLineDataToPass);
        
        for (int i = 0; i <= self.customLineDataToPass.count - 1; i++)
        {
            NSLog(@"[BEMSimpleLineGraph] Array number %i ",i);
            
            NSDictionary *specifyDict = self.customLineDataToPass[i];
            NSLog(@"[BEMSimpleLineGraph] Dictionary specified.");
            
            UIBezierPath *linePathe = [UIBezierPath bezierPath];
            linePathe.lineCapStyle = kCGLineCapButt;
            linePathe.lineWidth = [specifyDict[@"CUSTOM_LINE_WIDTH"] doubleValue];
            NSLog(@"[BEMSimpleLineGraph] Linewidth specified on linepathe.");
            
            CGPoint initialPoint = CGPointMake(0, [specifyDict[@"CUSTOM_LINE_START_POINT"] integerValue]);
            NSLog(@"[BEMSimpleLineGraph] Initialpoint specified.");
            CGPoint finalPoint = CGPointMake(self.frame.size.width, [specifyDict[@"CUSTOM_LINE_END_POINT"] integerValue]);
            NSLog(@"[BEMSimpleLineGraph] Finalpoint specified.");
            
            [linePathe moveToPoint:initialPoint];
            [linePathe addLineToPoint:finalPoint];
            
            [linePathe closePath];
            
            CAShapeLayer *customLineLayer = [CAShapeLayer layer];
            customLineLayer.frame = self.bounds;
            customLineLayer.path = averageLinePath.CGPath;
            customLineLayer.opacity = [specifyDict[@"CUSTOM_LINE_ALPHA"] doubleValue];
            NSLog(@"[BEMSimpleLineGraph] Custom Line Alpha specified.");
            customLineLayer.fillColor = nil;
            customLineLayer.lineWidth = [specifyDict[@"CUSTOM_LINE_WIDTH"] doubleValue];
            NSLog(@"[BEMSimpleLineGraph] Custom Line Width specified.");
            
            if (specifyDict[@"CUSTOM_LINE_DASHPATTERN"])
                customLineLayer.lineDashPattern = (NSArray *)specifyDict[@"CUSTOM_LINE_DASHPATTERN"];
            NSLog(@"[BEMSimpleLineGraph] Custom line dashpattern specified.");
            
            if (specifyDict[@"CUSTOM_LINE_COLOR"])
                customLineLayer.strokeColor = [specifyDict[@"CUSTOM_LINE_COLOR"] CGColor];
            else customLineLayer.strokeColor = [UIColor whiteColor].CGColor;
            NSLog(@"[BEMSimpleLineGraph] Custom line color specified.");
            
            if (self.animationTime > 0)
                [self animateForLayer:customLineLayer withAnimationType:self.animationType isAnimatingReferenceLine:NO];
            
            [self.layer addSublayer:customLineLayer];
            NSLog(@"[BEMSimpleLineGraph] Custom line added on sublayer.");
        }
    }
